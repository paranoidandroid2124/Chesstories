...
import lila.chessjudgment.analysis.assembly.{
  DefenseObligationChangeAssembler,
...
class DefenseObligationChangeTest extends munit.FunSuite:
  ...
  test("a non-check reference capture removes the played branch's sole exact recapturer"):
    ...
    val exact = deriveProofs("positive", positiveFen, referenceReplay, playedReplay) match
      case one :: Nil => one
      case other      => fail(s"expected one exact defense-obligation proof, found ${other.size}")

    assertEquals(exact.semantic.mechanism, DefenseObligationChangeMechanism.SoleRecapturerRemoval)
  ...
  test("exact demand ownership changes only the dependency fingerprint"):
    ...
    val first = DefenseObligationChangeAssembler.deriveDemanded(context, demand) match
      case one :: Nil => one
    ...
    val second = DefenseObligationChangeAssembler.deriveDemanded(
  ...
  test("forged comparison confidence, scope, or ancestry cannot open an L2 demand"):
    ...
    assert(DefenseObligationChangeProof.exactDemandRecord(
      demand,
      fact,
      root,
      referenceSource,
    ...
    assert(ForcedReplyResourceProof.exactDemandRecord(
      demand,
      fact,
      root,
      referenceSource,
  ...
  test("the certified family reaches the exact WrongMoveOrder consumer"):
    ...
    val proofPayload = proofRecord.payload.asInstanceOf[DefenseObligationChangeEvidence]
  ...
  private def lineRef(id: String, line: LineNodeRef, rootFen: String): EvidenceRef =
    EvidenceRef(
      ...
      position = PositionNodeRef(rootFen, 0, Some(White), Some("defense-obligation-root")),
      line = Some(line),
