  ...
  private def relativeCauseFlowDiagnostics(
    ...
    causeRecords.map { entry =>
      ...
      RelativeCauseFlowDiagnostic(
        ...
        claimCandidateFamilies = lifecycleCandidates.map(_.family).toSet,
        finalClaimFamilies = claims.map(_.family).toSet,
        directProofSourceIds = proof.directProof.sourceRefs.map(_.id).distinct.sorted,
        contrastProofSourceIds = proof.contrastProof.sourceRefs.map(_.id).distinct.sorted,
        contextSupportSourceIds = proof.contextSupport.sourceRefs.map(_.id).distinct.sorted,
...
object SemanticCoverageMetrics:
  private[qc] def pendingBranchReplyProbeRequests(
      requests: List[ProbeRequest],
      diagnostics: List[ProbeAdmissionDiagnostic]
  ): List[ProbeRequest] =
    val resolvedProbeIds = diagnostics.map(_.probeId).toSet
    requests.filterNot(request => resolvedProbeIds(request.id))
  ...
  def from(packet: EvidenceBackedJudgmentPacket): SemanticCoverageMetrics =
    ...
    val planTransitionWithoutSnapshotPairIds =
      packet.evidenceGraph.records.flatMap {
        case EvidenceRecord(ref, PlanTransitionEvidence(summary), parents) =>
          ...
          Option.unless(
            ownedEvents match
              case event :: Nil => event.planSequenceSummary.contains(summary) && transitionsBound
              case _            => false
          )(ref.id)
        case _ =>
          None
    ...
    val pendingProbeRequests =
      pendingBranchReplyProbeRequests(branchReplyProbeRequests, branchReplyProbeDiagnostics)
