...
object ThreatPressureAssessor:
  ...
  private def correctWithMultiPv(
    ...
    if multiPv.size < 2 then
      ...
      val candidateSetReliable =
        List(bestLine, secondLine).forall(line => JudgmentThresholds.engineBackedByDepth(line.depth, line.mate)) &&
          List(bestLine, secondLine).flatMap(_.moves.headOption).distinct.size == 2
      val evalLoss =
      ...
      val secondLineIsMate = bestLine.mate.forall(_ >= 0) && secondLine.mate.exists(_ < 0)
      ...
      if candidateSetReliable && hasSignificantThreat &&
  ...
  private def computeAggregates(
    ...
    val prophylaxisNeeded =
      !hasImmediate && hasStrategic &&
        maxWinPercentLoss.exists(_ >= JudgmentThresholds.SIGNIFICANT_THREAT_WP)
    val sharedDefense = threats.flatMap(_.bestDefense).distinct match
      case move :: Nil if threats.forall(_.defenseCount == 1) => Some(move)
      case _                                                  => None
    val totalDefenses = threats.map(_.defenseCount).maxOption.getOrElse(0)
