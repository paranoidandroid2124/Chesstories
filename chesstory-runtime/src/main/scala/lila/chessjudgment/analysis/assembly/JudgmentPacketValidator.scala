...
enum JudgmentPacketValidationIssueKind:
  ...
  case InvalidPlanTransitionSource
...
object JudgmentPacketValidator:
      ...
      val bindingIssues =
        record match
          ...
          case record @ EvidenceRecord(_, payload: PlanCausalEventEvidence, _) =>
            planCausalEventValidationIssues(packet.evidenceGraph, packet.probeDiagnostics, record, payload)
  ...
  private def planCausalEventValidationIssues(
      graph: TypedEvidenceGraph,
      probeDiagnostics: List[ProbeAdmissionDiagnostic],
    ...
    val matchingPressure = parents.exists {
      case EvidenceRecord(parentRef, _: PlanPressureEvidence, _) =>
        parentRef.line.contains(payload.rootLine) &&
          parentRef.position == payload.rootTransition.to
      case _ =>
        false
    }
    val matchingStructuralRecord = parents.collectFirst {
      case record @ EvidenceRecord(parentRef, structural: StructuralDeltaEvidence, _)
          if parentRef.line.contains(payload.rootLine) && structural.transition == payload.rootTransition =>
        record -> structural
    }
    val matchingStructural = matchingStructuralRecord.nonEmpty
    val structuralBindings = matchingStructuralRecord.toList.flatMap { case (structuralRecord, _) =>
      EvidenceObjectBinding.fromEvidenceRefs(graph, List(structuralRecord.ref))
    }
    val resultCarrierValid = payload.structuralConsequences.forall { consequence =>
      val deterrenceResult = payload.opponentResourceDeterrence.exists(_.consequence == consequence) &&
        payload.opponentResourceDeterrenceProofReady
      deterrenceResult || PlanCausalEventProof.consequenceBoundToRootMove(
        payload.rootLine,
        payload.rootMove,
        consequence,
        structuralBindings
    ...
    val developmentCarrierValid = payload.developmentChoices.forall(choice =>
      matchingStructuralRecord.exists { case (_, structural) => structural.developmentChoices.contains(choice) }
    )
    val lineParent = parents.collectFirst {
      case EvidenceRecord(parentRef, line: LineFactEvidence, _)
          if parentRef.line.contains(payload.rootLine) && line.line == payload.rootLine =>
        line
    }
    val continuationLineParent = payload.continuationSourceLine.flatMap { sourceLine =>
      parents.collectFirst {
        case EvidenceRecord(parentRef, line: LineFactEvidence, _)
            if parentRef.line.contains(sourceLine) && line.line == sourceLine && sourceLine.role == LineNodeRole.Threat =>
          line
      }
    }
    val continuationSourceValid = payload.continuationSourceLine match
      case None => true
      case Some(sourceLine) =>
        payload.episode.nonEmpty &&
          sourceLine.role == LineNodeRole.Threat &&
          continuationLineParent.exists(line =>
            line.lineReplaySteps.headOption.exists(step =>
              PrincipalVariationEvidence.sameBoardState(step.fenBefore, payload.rootTransition.to.fen)
            )
          )
    val matchingTransition = parents.exists {
      case EvidenceRecord(parentRef, MoveTransitionEvidence(move, from, to), _) =>
        parentRef.scope == payload.rootTransition.role.scope &&
          EvidenceRef.sameMove(move, payload.rootMove) &&
          from == payload.rootTransition.from &&
          to == payload.rootTransition.to
      case _ =>
        false
    }
    val normalizedRootMove = EvidenceRef.normalizeMove(payload.rootMove)
    val identityValid =
      normalizedRootMove.length >= 4 &&
        EvidenceRef.sameMove(payload.identity.rootMove, normalizedRootMove) &&
        payload.identity.actorFrom.contains(normalizedRootMove.take(2)) &&
        payload.identity.actorTo.contains(normalizedRootMove.slice(2, 4))
    def causalEpisodeShapeValid(episode: PlanCausalEpisode): Boolean =
      val steps = episode.chronologicalSteps
      episode.causalEpisodeProven &&
        episode.dependencies.distinct.size == episode.dependencies.size &&
        episode.dependencies.forall(dependency =>
          steps.contains(dependency.from) &&
            steps.contains(dependency.to) &&
            dependency.planConnectionProven
        ) &&
        episode.responses.forall(response => episode.planSteps.contains(response.trigger) && response.proven)
    val episodeProofValid = payload.episode match
      case None =>
        payload.branchWitnesses.isEmpty && payload.continuationSourceLine.isEmpty
      case Some(episode) =>
        causalEpisodeShapeValid(episode) &&
          episode.root.identity == payload.identity &&
          EvidenceRef.sameMove(episode.root.moveUci, payload.rootMove) &&
          PrincipalVariationEvidence.sameBoardState(episode.root.step.fenBefore, payload.rootTransition.from.fen) &&
          PrincipalVariationEvidence.sameBoardState(episode.root.step.fenAfter, payload.rootTransition.to.fen) &&
          episode.root.perspective == payload.perspective &&
          episode.root.structuralConsequences.forall(payload.structuralConsequences.contains) &&
          episode.root.developmentChoices.forall(payload.developmentChoices.contains)
    val deterrenceCarrierValid = payload.opponentResourceDeterrence.forall { proof =>
      payload.opponentResourceDeterrenceProofReady &&
        (proof.resourceLine :: proof.comparisons.map(_.resourceLine)).forall(sourceLine =>
          parents.exists {
            case EvidenceRecord(parentRef, line: LineFactEvidence, _) =>
              parentRef.line.contains(sourceLine) && line.line == sourceLine
            case _ =>
              false
          }
        )
    }
    val eventAuthorityValid =
      ref.confidence == EvidenceConfidence.Heuristic || ref.confidence == EvidenceConfidence.Mixed
    def branchWitnessValid(expectedEpisode: PlanCausalEpisode, witness: PlanCausalBranchWitness): Boolean =
      val linePayload = parents.collectFirst {
        case EvidenceRecord(parentRef, line: LineFactEvidence, _)
            if parentRef.line.contains(witness.line) && line.line == witness.line && witness.line.role == LineNodeRole.Threat =>
          line
      }
      val expectedResult = payload.publicTailExpectedResult
      val expectedRequiredPly = expectedResult
        .map((sourceEvent, _) => sourceEvent.step.ply - expectedEpisode.root.step.ply)
        .getOrElse(expectedEpisode.requiredPlyOffset)
        .max(1)
      val canonicalObservation = payload.publicTailExpectedResultAssessment.toList
        .flatMap(_.observations)
        .find(_.line == witness.line)
      val observedConsequences = witness.observedEpisode.toList.flatMap(
        _.continuationsEnabledByRoot.flatMap(PlanCausalEpisode.resultConsequences)
      ).distinct
      val admittedProbeAuthorityValid = probeDiagnostics.exists(diagnostic =>
        diagnostic.status == ProbeAdmissionStatus.Admitted &&
          diagnostic.probeId == witness.sourceProbeId &&
          diagnostic.horizon.flatMap(ProbeHorizon.plyOffset).contains(witness.certifiedHorizonPlyOffset) &&
          diagnostic.variationHash.exists(_.trim.nonEmpty)
      )
      linePayload.exists(line =>
        val proofThroughPly = line.lineReplayCount.min(witness.certifiedHorizonPlyOffset)
        val observedLine = line.lineReplaySteps.take(proofThroughPly)
        val terminalStep = Option.when(observedLine.size == line.lineReplayCount)(observedLine.lastOption).flatten
        val terminalOutcome = terminalStep.flatMap(step =>
          PlanCausalEventProof.terminalOutcomeAt(step.fenAfter, expectedEpisode.root.perspective)
        )
        canonicalObservation.exists(observation =>
            val expectedObservedPlyOffset = observation
              .realizationPlyOffset(expectedEpisode.root)
              .orElse(terminalOutcome.map(_ => proofThroughPly))
              .getOrElse(proofThroughPly)
            witness.sourceProbeId.trim.nonEmpty &&
            admittedProbeAuthorityValid &&
            witness.requiredPlyOffset == expectedRequiredPly &&
            witness.certifiedHorizonPlyOffset > 0 &&
            witness.observedThroughPlyOffset == proofThroughPly &&
            witness.observedPlyOffset == expectedObservedPlyOffset &&
            witness.observedEpisode.forall(observed =>
              observed.root == expectedEpisode.root && causalEpisodeShapeValid(observed)
            ) &&
            witness.observedConsequences.toSet == observedConsequences.toSet &&
            witness.outcome == observation.outcome &&
            witness.realizationMatch == observation.realizationMatch &&
            witness.realizationMove == observation.realizationMove &&
            witness.terminalOutcome == terminalOutcome &&
            witness.terminalPlyOffset == terminalOutcome.map(_ => observedLine.size) &&
            witness.terminalStep == Option.when(terminalOutcome.nonEmpty)(terminalStep).flatten
        )
      )
    val branchProofValid =
      payload.episode match
        case None => payload.branchWitnesses.isEmpty
        case Some(expectedEpisode) =>
          payload.branchWitnesses.map(_.line).distinct.size == payload.branchWitnesses.size &&
            payload.branchWitnesses.map(_.sourceProbeId).distinct.size <= 1 &&
            payload.branchWitnesses.forall(branchWitnessValid(expectedEpisode, _))
    ...
    val eventBindingValid =
      recordBindingValid &&
        identityValid &&
        continuationSourceValid &&
        deterrenceCarrierValid &&
        resultCarrierValid &&
        developmentCarrierValid &&
        eventAuthorityValid &&
        resultShapeValid
    List(
      ...
      Option.when(!matchingPressure)(
        JudgmentPacketValidationIssue(
          JudgmentPacketValidationIssueKind.MissingPlanCausalPressureParent,
          ref.id,
          Some(ref)
        )
      ),
      ...
      Option.when(lineParent.isEmpty)(
        JudgmentPacketValidationIssue(JudgmentPacketValidationIssueKind.MissingPlanCausalLineParent, ref.id, Some(ref))
      ),
  ...
  private def planTransitionValidationIssues(
    ...
    val matchingEventSource = eventParents.find { case (_, event) => event.planSequenceSummary.contains(summary) }
    val valid =
      matchingEventSource.exists { case (eventRecord, event) =>
        record.parents == List(eventRecord.ref) &&
        eventParents.size == 1 &&
        ...
        record.ref.confidence == eventRecord.ref.confidence &&
        StrategicMechanismEvidence.planTransitionCanSupportPlan(summary)
      }
    Option
      .when(!valid)(
        JudgmentPacketValidationIssue(
          JudgmentPacketValidationIssueKind.InvalidPlanTransitionSource,
          record.ref.id,
          Some(record.ref)
        )
      )
  ...
  private def validProbeRequest(request: ProbeRequest): Boolean =
    ...
    val depthValid =
      if request.purpose.exists(ProbePurpose.isEndgameTablebase) then request.depth == 0
      else request.depth > 0
    ...
    val tablebaseValid =
      request.purpose.filter(ProbePurpose.isEndgameTablebase).forall(_ => validEndgameTablebaseProbeRequest(request))
    idValid && fenValid && depthValid && purposeValid && requiredSignalsValid && movesValid && branchValid && tablebaseValid

  private def validBranchReplyProbeRequest(request: ProbeRequest): Boolean =
    if request.objective.contains(CounterResourceProbeBinding.Objective) then
      validCounterResourceProbeRequest(request)
    else
      validReplyMultiPvProbeRequest(request)

  private def validReplyMultiPvProbeRequest(request: ProbeRequest): Boolean =
    val signals = request.requiredSignals.map(_.trim).filter(_.nonEmpty).toSet
    Set("replyLines", "depth", "purpose", "variationHash", "horizon").subsetOf(signals) &&
      request.candidateMove.exists(validUciMove) &&
      request.multiPv.contains(BranchReplyProbeBinding.requiredReplyCount(request.fen)) &&
      request.multiPv.exists(_ > 0) &&
      request.horizon.flatMap(BranchReplyProbeBinding.horizonPlyOffset).nonEmpty &&
      request.depthFloor.exists(floor => floor > 0 && floor <= request.depth) &&
      request.variationHash.exists(_.trim.nonEmpty)

  private def validCounterResourceProbeRequest(request: ProbeRequest): Boolean =
    val signals = request.requiredSignals.map(_.trim).filter(_.nonEmpty).toSet
    val resource = request.opponentResourceMove.filter(validUciMove)
    CounterResourceProbeBinding.RequiredSignals.toSet.subsetOf(signals) &&
      request.candidateMove.exists(validUciMove) &&
      resource.nonEmpty &&
      request.moves == resource.toList &&
      request.multiPv.contains(1) &&
      request.horizon.isEmpty &&
      request.depthFloor.exists(floor => floor > 0 && floor <= request.depth) &&
      request.variationHash.exists(_.trim.nonEmpty)

  private def validEndgameTablebaseProbeRequest(request: ProbeRequest): Boolean =
    val signals = request.requiredSignals.map(_.trim).filter(_.nonEmpty).toSet
    val terminal = Fen.read(Standard, Fen.Full(request.fen))
    val comparison = request.comparisonFen.flatMap(fen => Fen.read(Standard, Fen.Full(fen)))
    val legalMoves = terminal.toList.flatMap(_.legalMoves.map(_.toUci.uci)).distinct.sorted
    Set("tablebase", "purpose", "variationHash").subsetOf(signals) &&
      request.depth == 0 && request.multiPv.isEmpty && request.depthFloor.isEmpty &&
      request.candidateMove.isEmpty && request.horizon.isEmpty && request.variationHash.exists(_.trim.nonEmpty) &&
      terminal.exists(_.board.occupied.count <= 7) && comparison.nonEmpty &&
      request.moves.map(_.trim.toLowerCase).distinct.sorted == legalMoves
