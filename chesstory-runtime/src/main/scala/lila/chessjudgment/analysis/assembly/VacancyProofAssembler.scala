...
/** Sole graph owner for exact vacancy-to-occupation results. */
private[chessjudgment] object VacancyEnablesOccupationAssembler:
  ...
  private[assembly] def fromDemand(
    ...
    if demand.vacancyEnablesOccupationSeeds.isEmpty then Nil
    else
      val input = demand.input
      val existingOwners =
        context.evidenceGraph.recordsFor(input.comparison.referenceLine).collect {
      ...
      val existing = existingOwners.map {
        case record @ EvidenceRecord(_, payload: VacancyEnablesOccupationEvidence, _) =>
          (payload.dependencyId, payload.resultSet, record)
        case _ =>
          throw IllegalStateException("a vacancy-enables-occupation lookup returned a foreign payload")
      }
      ...
      ExactCausalProofOwnerReuse.resolve(
        ...
        derive(input, demand.vacancyEnablesOccupationSeeds).map(exact =>
          exact.dependency.value -> exact
        )
      ) { (exact, resultSet) =>
        val payload = VacancyEnablesOccupationEvidence(
          semantic = exact.semantic,
          occurrence = exact.occurrence,
          dependencyFingerprint = exact.dependency.value,
          resultSet = resultSet,
        ...
        EvidenceRecord(
          ref = EvidenceRef(
            id = allocator.evidenceId(
              s"causal-proof:vacancy-enables-occupation:${payload.semanticId}:${payload.occurrenceId}:${payload.dependencyId}:$pathIds"
            ),
            producer = EvidenceProducer.CausalProofProducer,
            layer = EvidenceLayer.CausalProof,
            position = exact.parentSources
              ...
              .map(_.position)
              .getOrElse(
                throw IllegalStateException("a vacancy proof lost its reference line parent")
              ),
            line = Some(exact.occurrence.referenceLine),
            scope = exact.occurrence.referenceLine.role.scope,
            confidence = EvidenceConfidence.LegalReplayVerified
  ...
  private def derive(
    ...
    val certified = VacancyEnablesOccupationProof.deriveChangedDependencies(
      input.comparison.referenceLine,
      input.comparison.candidateLine,
      input.referenceSource,
      input.playedSource,
    ...
    certified.groupBy(_.semantic.semanticId).foreach { case (semanticId, occurrences) =>
      require(
        occurrences.map(_.semantic).distinct.size == 1,
        s"vacancy-enables-occupation semantic id '$semanticId' resolved to conflicting proofs"
      )
    }
    require(
      certified.map(_.occurrence.occurrenceId).distinct.size == certified.size,
      "one exact vacancy-enables-occupation occurrence may be produced only once"
    )
    certified.sortBy(_.occurrence.occurrenceId)
