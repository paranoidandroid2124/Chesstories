...
object MoveReviewInputNormalizer:
    ...
    probes.foreach { probe =>
      threatBranchSeed(beforeFen, rootLines, probe) match
        ...
        case Right(seed) =>
          ...
          val ranked =
            preferredRankedLines(
              seed.branchFen,
              seed.lines.zipWithIndex.filter(_._1.moves.nonEmpty),
              side,
              requireTerminalScoreConsistency = false
            )
              .map(_._1)
          val legalLines = ranked.filter(line => legalLine(seed.branchFen, line))
          val requiredLineCount =
            if seed.opponentResourceMove.nonEmpty then 1
            else BranchReplyProbeBinding.requiredReplyCount(seed.branchFen)
          val topLegalLines = legalLines.take(requiredLineCount)
          val topLinesDepthReady = topLegalLines.forall(_.depth >= depthFloor)
          val topLinesTerminalScoreConsistent = topLegalLines.forall(terminalScoreConsistent(seed.branchFen, _))
          ...
          if requiredLineCount > 0 &&
            topLegalLines.size == requiredLineCount &&
            topLinesDepthReady &&
            topLinesTerminalScoreConsistent &&
            opponentResourceLineBound
          then
            val branchLines = topLegalLines.map { line =>
              val rank = nextRank
              nextRank += 1
              NormalizedCandidateLine(
            ...
            val reason =
              if requiredLineCount == 0 then "BRANCH_POSITION_TERMINAL"
              else if legalLines.size < requiredLineCount then "INSUFFICIENT_LEGAL_REPLY_LINES"
  ...
  private def threatBranchSeed(
    ...
    probe.purpose.filter(ProbePurpose.isBranchReply) match
      ...
      case Some(purpose) =>
        ...
        else (branchFen, scoredLines, probedMove) match
          case (None, _, _) =>
                ...
                if matchingMoveLines.isEmpty then
                  ...
                  matchingMoveLines.flatMap(line =>
                    branchHash(beforeFen, line, move, opponentResourceMove, certifiedHorizonPlyOffset)
                      .filter(probe.variationHash.contains)
                      .map(line -> _)
                  ).headOption match
                    case None =>
                      Left(baseDiagnostic(List("VARIATION_HASH_MISMATCH")))
                    case Some((rootLine, expectedHash)) =>
  ...
  private def branchHash(
    ...
    opponentResourceMove match
      case Some(resourceMove) =>
        Some(CounterResourceProbeBinding.variationHash(
          rootFen = beforeFen,
          role = line.role,
          rootMove = probedMove,
