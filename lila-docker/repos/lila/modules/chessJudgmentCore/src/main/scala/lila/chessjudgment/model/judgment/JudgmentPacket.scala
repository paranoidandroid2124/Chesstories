    ...
    MoveMeaningSurfaceTarget(
      squares =
        (if structuredCarriers then targetCarriers.filter(_.kind == "Square").map(_.value) else claim.targetSquares)
          ...
          .sorted,
      files =
        (if structuredCarriers then targetCarriers.filter(_.kind == "File").map(_.value) else claim.targetFiles)
          .flatMap(cleanFile)
          .distinct
          .sorted,
      pieces =
        (
          if targetCarriers.nonEmpty then claim.targetPieces ++ targetCarriers.filter(_.kind == "Piece").map(_.value)
          else if structuredCarriers then Nil
          else claim.targetPieces ++ routePieces(claim.routeIdentityParts)
        ).flatMap(cleanPiece).distinct.sorted
    )
...
object MoveMeaningSurface:
  ...
  def publicSurfaces(view: MoveJudgmentView): List[MoveMeaningSurface] =
    publicIdeaChainSurfaceClaimsWithEvidence(view)
  ...
  private def publicIdeaChains(verdict: MoveMeaningSurfaceVerdict, surfaces: List[MoveMeaningSurface]): List[JsObject] =
    ...
    publicIdeaChainSubjectSpecs(verdict, surfaces).flatMap { (subject, subjectMove, pvRolePrefix) =>
      ...
      if evidenceSurfaces.isEmpty || !admittedSubjects.contains(subject) then Nil
        ...
        val candidateDisplaySemantics = chainSurfaces
        ...
        val displaySemantics =
          candidateDisplaySemantics.filterNot(surface => displayRouteOnlySurfaceCoveredByPublicMeaning(surface, candidateDisplaySemantics))
        ...
        if displaySemantics.isEmpty || displayRouteOnlyChainWithoutPurpose then Nil
          ...
          List(
            Json.obj(
              ...
              "move_quality" -> verdict.moveQuality,
              "subject" -> subject,
              "move_semantics" -> displaySemantics.map(publicIdeaChainMoveSemanticJson),
              "proof_levels" -> displaySemantics.map(_.evidence.proofLevel).distinct,
        ...
        other != surface &&
          ...
          (
            other.idea.code == surface.idea.code &&
              ...
              pawnStructureAdvanceIdea(other.idea.code) &&
                (
                  surface.target.squares.intersect(other.target.squares).nonEmpty ||
                    surface.target.files.intersect(other.target.files).nonEmpty ||
                    surface.evidence.sourceIds.intersect(other.evidence.sourceIds).nonEmpty
    ...
    displayOnlySurfaceEvidence(surface) &&
      ...
      surfaces.exists(other =>
        other != surface &&
          ...
          other.idea.code == surface.idea.code &&
          other.evidence.proofLevel == surface.evidence.proofLevel &&
          !multiPieceTargetPressureSurface(other) &&
          other.target.squares == surface.target.squares &&
          other.target.files == surface.target.files
      )
  ...
  private def multiPieceTargetPressureSurface(surface: MoveMeaningSurface): Boolean =
    surface.idea.code == "target_pressure" &&
      surface.target.pieces.map(_.toLowerCase).filterNot(piece => piece == "king" || piece == "pawn").distinct.size > 1
  ...
  private def displayLineUnlockSurfaceCoversAuxiliaryTargetPressure(surface: MoveMeaningSurface, surfaces: List[MoveMeaningSurface]): Boolean =
    ...
    val pawnAdvanceFallback =
      surface.unit == PositionPlanTechniqueUnit.PlanOptionSet &&
        sameFilePawnAdvanceMove(surface.moveUci) &&
        (
          surfaceHasCarrierRole(surface, "pawn_advance_destination") ||
  ...
  private def fromClaim(
    ...
    val targetWithContext =
      flankDestination match
        ...
        case Some(destination) if pawnStructureAdvanceSurfaceIdea && kingPressureCarrier && !pawnTensionLabelClaim && idea != "flank_pawn_pressure" =>
          baseTarget.copy(
            squares = List(destination),
            files = List(destination.take(1))
          )
...
object MoveMeaningClaim:
  ...
  private def fromDetail(
    ...
    val bridgedPlanCauseFrames =
      causeFramesById.values.flatten.toList
        ...
        .distinctBy(_.causeEvidenceIds)
    val bridgedTacticalProofCauseFrames =
      causeFramesById.values.flatten.toList
        .filter(frame => sameRootTacticalProofOwnedCauseBridgeCandidate(frame, verdict, detail, objectSignatures))
        .distinctBy(_.causeEvidenceIds)
    val linkedCauseFrames =
      (directLinkedCauseFrames ++ bridgedRouteCauseFrames ++ bridgedPlanCauseFrames ++ bridgedTacticalProofCauseFrames)
        .distinctBy(_.causeEvidenceIds)
    val bridgedCauseIds =
      (bridgedRouteCauseFrames ++ bridgedPlanCauseFrames ++ bridgedTacticalProofCauseFrames).flatMap(_.causeEvidenceIds).toSet
            ...
            val optionRoleCompatibleCauseFrames =
              linkedCauseFrames.filter(linkedFrame =>
                causeFrameOwnsMeaningClaim(
                  ...
                  sameRootTacticalProofOwnedCauseBridge(
                    linkedFrame,
                    verdict,
                    detail,
                    objectSignatures,
  ...
  private def sameRootTacticalProofOwnedCauseBridgeCandidate(
      frame: MoveJudgmentCauseFrame,
      verdict: MoveJudgmentVerdictFrame,
      detail: PositionPlanTechniqueSemanticDetail,
      objectSignatures: List[String]
  ): Boolean =
    (frame.proofRelationKinds.nonEmpty || frame.proofLineConsequences.exists(LineConsequenceKind.tacticalDriver)) &&
      frame.hasOwnedTacticalProof &&
      nonLossMeaningVerdict(verdict.verdict) &&
      reasonGradeCauseFrame(frame) &&
      crossComparisonOwnedRootTier(frame) &&
      tacticalProofDetailCanCarry(detail) &&
      detailCanOwnCrossComparisonCause(detail) &&
      (causeFrameObjectOverlapsDetail(frame, objectSignatures) || frame.proofLineConsequences.exists(LineConsequenceKind.tacticalDriver))

  private def tacticalProofDetailCanCarry(detail: PositionPlanTechniqueSemanticDetail): Boolean =
    detail.unit == PositionPlanTechniqueUnit.PieceRerouteRoute ||
      (
        detail.unit == PositionPlanTechniqueUnit.StructuralTransformation &&
  ...
  private def sameRootTacticalProofOwnedCauseBridge(
      frame: MoveJudgmentCauseFrame,
      verdict: MoveJudgmentVerdictFrame,
      detail: PositionPlanTechniqueSemanticDetail,
      objectSignatures: List[String],
    ...
    sameRootTacticalProofOwnedCauseBridgeCandidate(frame, verdict, detail, objectSignatures) &&
      causeFrameLineOwnsClaimMove(frame, verdict, claimLineRole, claimMove) &&
      detailOwnsClaimMove(detail, objectSignatures, claimMove) &&
      causeFramePolarityCompatibleWithMeaning(frame, verdict, detail, claimRole)
  ...
  private def opponentRestrictionHasBoardCarrier(claim: MoveMeaningClaim): Boolean =
    ...
    claim.unit == PositionPlanTechniqueUnit.SpacePreventionResourceDenial &&
      ...
      (
        claim.routeIdentityParts.exists(_.toLowerCase.contains(":color-complex-safe")) ||
          claim.objectBindingSignatures.exists(_.toLowerCase.contains("opponentmobilityrestriction")) ||
          claim.axisKey.exists(_.toLowerCase.contains("opponent-mobility-restriction")) ||
          claim.label.exists(label =>
            label.equalsIgnoreCase("opponent-mobility-restriction") ||
              label.equalsIgnoreCase("opponent-diagonal-restriction")
          )
      )
