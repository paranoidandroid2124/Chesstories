...
final class Limiters(using Executor, lila.core.config.RateLimit):
  ...
  val moveReviewJobCreation =
    RateLimit[IpAddress](credits = 120, duration = 1.minute, key = "analyse.moveReview.create.ip", log = false)
  val moveReviewJobAccess =
    RateLimit[IpAddress](credits = 960, duration = 1.minute, key = "analyse.moveReview.access.ip", log = false)
